...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${RAZOR_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    RAZOR_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/razor/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the RAZOR github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var RAZOR_VERSION)
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $RAZOR_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.razor.device | grep -q "$RAZOR_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $RAZOR_FIXUP_COMMON_OUT ]; then
